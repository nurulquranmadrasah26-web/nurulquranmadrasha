...
(function () {
 ...
 var label = (a.textContent || '').replace(/[+＋]/g, '').trim();
    ...
    [{ view: 'dashboard', label: 'ড্যাশবোর্ড' }, { view: 'profile', label: 'প্রোফাইল' }, { view: '__qard', label: 'কর্জে হাসানা' }]
  ...
  function buildDrawer(items) {
    ...
    items.forEach(function (it) {
      ...
      b.addEventListener('click', function () {
        close();
        if (it.view === '__qard' && typeof window.nqOpenQardPage === 'function') window.nqOpenQardPage();
        else if (typeof window.goto === 'function') window.goto(it.view);
      });
      grid.appendChild(b);
    });
